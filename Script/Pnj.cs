...
public class Pnj : MonoBehaviour
{
	...
	if(ModeRandom){PositionPoint = Random.Range(0,PointDeplacement.Length);}
    ...
    void Update()
    {
		...
		m_Animator.SetFloat("Forward", m_ForwardAmount*SyncroVitesse,SyncroAnnimation,Time.deltaTime);
			if(OnMaison){
				...
				if(MaisonfadeTransition > 0.05f){MaisonfadeTransition -= Time.deltaTime * 0.7f;
				}else{
		...
		if(tempsJourNuit <= 0.255f){TimerChangePositionVar = 0.5f;}
		PhasesMaison = false;
		OnMaison = false;
	...
	if(!OnDialogVar && PhasesMaison){
	Agent.SetDestination(Maison.transform.position);
	}
	OnDialogVar = true;
	}else{
	if(OnDialogVar && !OnMaison){
