...
{
	[HideInInspector]
    public int[] EmplacementID,EmplacementNombreObject,EmplacementIDMaterial,EmplacementNombreObjectMaterial,EmplacementNombreObjectSpecial1,EmplacementNombreObjectSpecial2;
	public ClassInfoItem[] InfoItem;
	...
	public bool selectedID;
	public int lastIDSelected,NewIDSelected;
	...
	void VerificationInventaire(){
		if(!Material){
			for(int i = 0;i < EmplacementID.Length;i++){
				...
				if(EmplacementID[i] == IdLoot){ObjectMax = true;break;}
			}
		}else{
			for(int i = 0;i < EmplacementIDMaterial.Length;i++){
				if(EmplacementIDMaterial[i] == 0){
				...
				if(EmplacementIDMaterial[i] == IdLoot){ObjectMax = true;break;}
			}
		}
	}
	void Drop(){
